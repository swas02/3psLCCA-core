...
def calculate_road_user_costs(traffic_input, wpi, debug=False):
    ...
    voc_final = congestion.calculate_total_adjusted_costs(voc_raw, traffic_input, debug)

    # 4. Value of Time (VOT)
    vot = value_of_time.calculate_additional_time_cost(traffic_input, wpi, debug)

    # 5. Total Carbon Emission
    tce = total_carbon_emission.calculate_total_carbon_emission(traffic_input["vehicle_data"], debug)

    return {
        "accident_cost": ac,
        "vehicle_operation_cost": voc_final, # Simplified: Congestion is now built-in
        "value_of_time": vot,
