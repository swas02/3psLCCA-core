...
class VOCPostProcessor:
    ...
    def _get_strict_multiplier(self, category: str, vehicle_type: str, sub_key: str = None) -> float: # type: ignore
        ...
        if category in self.wpi:
            ...
            if isinstance(block, dict):
                if v_key in block:
                    ...
                    raise ValueError(f"CRITICAL: Key '{v_key}' or '{sub_key}' missing in {path}")
            else:
                val = block
        ...
        if not isinstance(val, (int, float)):
            raise ValueError(f"CRITICAL: Expected number at {path}, but got {type(val).__name__}")
    ...
    def _apply_adjustment(self, cost_base: Dict[str, float], mult: float, path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ValueError(f"CRITICAL: Base cost keys (IT/ET/VALUE) missing for {path}")
    ...
    def process(self, voc_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for vt in vehicle_type_list:
            if vt not in voc_data: continue
            ...
            b_tyre = {
                ...
                c.ET: (t_info[c.ET] * t_info[c.NUMBER_OF_WHEELS]) / dist_s["tyre_life"][c.VALUE]
            }
            dc["tyreCost"] = self._apply_adjustment(b_tyre, self._get_strict_multiplier("tyreCost", vt), f"tyreCost for {vt}")

            # --- Fuel & Lubricants ---
            f_cons = dist_s["fuel_consumption"]
            ratio = petrolToDieselRatio[vt]
            
            p_mult = self._get_strict_multiplier("fuelCost", vt, sub_key=c.PETROL)
            d_mult = self._get_strict_multiplier("fuelCost", vt, sub_key=c.DIESEL)
            ...
            for oil in [c.ENGINE_OIL, c.OTHER_OIL, c.GREASE]:
                o_mult = self._get_strict_multiplier("fuelCost", vt, sub_key=oil)
                fac = 1000 if oil == c.ENGINE_OIL else 10000
                o_base = {c.VALUE: (dist_s[oil][c.VALUE] * tableC1.petroleum_products_costs[oil][c.IT]) / fac}
                dc[oil] = self._apply_adjustment(o_base, o_mult, f"fuelCost -> {oil}")

            # --- Maintenance ---
            m_mult = self._get_strict_multiplier("spareParts", vt)
            dc[c.SP] = self._apply_adjustment({c.IT: dist_s[c.SP][c.IT], c.ET: dist_s[c.SP][c.ET]}, m_mult, "spareParts")
            dc["maintenance_labour"] = self._apply_adjustment({c.VALUE: dist_s["maintenance_labour"][c.VALUE]}, m_mult, "labour")

            # --- Time Related ---
            fd_mult = self._get_strict_multiplier("fixedDepreciation", vt)
            tc["fixed_cost"] = self._apply_adjustment({c.IT: time_s["fixed_cost"][c.IT], c.ET: time_s["fixed_cost"][c.ET]}, fd_mult, "fixedDepreciation")
            
            pc_mult_crew = self._get_strict_multiplier("passengerCrewCost", vt, sub_key="Crew Cost")
            pc_mult_pass = self._get_strict_multiplier("passengerCrewCost", vt, sub_key="Passenger Cost")
            
            tc["crew_cost"] = self._apply_adjustment({c.VALUE: time_s["crew_cost"][c.VALUE]}, pc_mult_crew, "crewCost")
            tc["passenger_time_cost"] = self._apply_adjustment({c.VALUE: time_s["passenger_time_cost"][c.VALUE]}, pc_mult_pass, "passCost")

            ch_mult = self._get_strict_multiplier("commodityHoldingCost", vt)
            tc["commodity_holding_cost"] = self._apply_adjustment({c.VALUE: time_s["commodity_holding_cost"][c.VALUE]}, ch_mult, "commodityHolding")
...
def calculate_total_cost(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for cost_type in ["distanceCost", "timeCost"]:
        if cost_type not in data: continue
        total_cost[cost_type] = {}
        for vt, components in data[cost_type].items():
            total_cost[cost_type][vt] = {c.IT: 0.0, c.ET: 0.0}
            for comp_name, comp_values in components.items():
                if comp_name == "WPI_Debugger" or not isinstance(comp_values, dict): continue
                total_cost[cost_type][vt][c.IT] += comp_values.get(c.IT, comp_values.get(c.VALUE, 0.0))
                total_cost[cost_type][vt][c.ET] += comp_values.get(c.ET, comp_values.get(c.VALUE, 0.0))
